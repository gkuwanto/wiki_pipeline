...
def get_max_file_num(language, folder):
    file_list = listdir(f'{path}/wiki-{language}/{folder}')
    if len(file_list):
...
def get_list_of_folder(language):
    return sorted([f for f in listdir(f"{path}/wiki-{language}") if f[0] != '.'])
...
for fol in folder_list[folder_start:]:
    ...
    while num <= max_num:
        dic = get_first_paragraf(
            f"{path}/wiki-{language}/{fol}/{'wiki_'+str(num).zfill(2)}", dictionary=dic)
